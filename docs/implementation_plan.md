...
- **狀態管理與 UI 更新 (詳細網頁功能要求)**：
  ...
  3. **設定本次點餐與開放時間 (管理員專屬分頁)**：
     - 從 `Menu` 表撈出不重複的餐廳名稱列表。
     - 管理員可在該分頁勾選今天選用的餐廳。
     - **新增時間設定：** 管理員可設定「開放訂餐開始時間」與「結束時間」，並且格式強化為**包含「年月日與時分」 (datetime-local)**。
     - **重設開放設定按鈕：** 管理區提供「重設」按鈕。若仍在設定的點餐時間內，則跳出二次警告。確認重設後，將同步清除 `TodayConfig`（今日設定）與 `Orders`（所有的訂單），且連動點餐區清空。
     - 設定完成後寫入 `TodayConfig` 工作表（A欄紀錄餐廳，C1、D1 紀錄完整日期時間），並強制觸發點餐區的再次渲染連動。
     - **管理菜單功能：** 顯示目前 `Menu` 表的列表，提供「新增餐點」、「編輯餐點」與「刪除按鈕」。分類欄位使用 `<datalist>` 支援「下拉選擇既有分類」或「直接輸入新分類」。修改完成後覆寫整張 `Menu` 表。
  4. **點餐介面 (預設點餐區分頁)**：
     - 讀取 `TodayConfig` 取得今日餐廳名稱與**訂餐時間範圍 (含日期)**。
     - **時間驗證機制：** 利用準確的 `Date` 物件比對目前時間是否在開放的起訖日內。若當下時間不在範圍內，按鈕反灰不允許點餐，點擊時跳出警告。
     - 從 `Menu` 中過濾出「屬於今日餐廳」的餐點，並以單色優雅的卡片顯示（含品名、單價、備註、點餐按鈕）。
     - **個人訂單區塊：** 畫面上方或同頁的獨立區塊中，會自動抓取 `Orders` 並過濾出「該使用者的所有當日訂單」。使用者可一目了然看見自己剛才點了什麼。
     - **取消訂單：** 個人訂單清單中會附有「取消」按鈕（同樣受開放時間驗證限制）。點擊後，系統會計算出該筆訂單在試算表中的精確「列數 (Row Index)」，並針對該單一列執行 `clear` 操作，這能避免多人同時送單時整張表覆寫帶來的資料遺失風險。
  5. **送出訂單**：
     - 點擊後檢查時間是否超時，若合法則將資料（時、Email、餐廳、餐點、金額、備註）寫入 `Orders` 工作表最下方，並顯示提示訊息與重新渲染「個人訂單區塊」。
  6. **確認訂單 (確認訂單區分頁)**：
     - 列出今天的餐點列表（自動忽略被刪除的空白列）。提供「一鍵複製」按鈕，格式化成清單。
  7. **管理員專區**：
